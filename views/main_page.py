...
st.title("Analisis Pola Pembelian Emas dengan FP-Growth")
...
if st.button("Analisis Pola Pembelian"):
    ...
    st.subheader("Proses Analisis FP-Growth")
    ...
    st.subheader("Daftar Jumlah Transaksi per Item")

    item_counts = df_clean[col_item].value_counts().reset_index()

    item_counts.columns = ["Nama Item", "Jumlah Transaksi"]

    st.dataframe(item_counts, use_container_width=True)

    st.subheader("Hasil Pola Pembelian")
